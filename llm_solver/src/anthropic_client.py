...
class AnthropicWrapper:
    ...
    def send_prompt(self, contents: typing.List[ChatMessage]) -> ChatMessage:
        if len(contents) == 0:
            raise ValueError("Received empty prompt contents")
        messages = [msg.to_dict() for msg in contents]
        cache_key = (self.system_prompt, contents)
        cached_res = cache.get(cache_key)
        if cached_res:
            logging.debug("cache hit: %s", self.system_prompt)
            return cached_res
        logging.info("Cache miss")
        logging.debug("Cache miss for: %s(%d)", self.system_prompt, len(messages))
...
def extract_json_from_prompt_text_block(text_block: str, expected_schema=None) -> dict:
    logging.debug("extracting json from: %s", text_block)
    res = {}
    ...
    logging.debug("Extracted json: %s", res)
    if expected_schema:
        try:
            jsonschema.validate(res, expected_schema)
        except jsonschema.ValidationError as vex:
            logging.error("Generated res has an unexpected schema: %s", vex, exc_info=True)
            raise SchemaValidationError(f"Generated res has an unexpected schema: {vex}")
    return res
